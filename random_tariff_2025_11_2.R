...
# general steady state equilibrium quantities
...
# general steady state equilibrium consumer prices
...
# separating profits into domestic and foreign
...
ggplot(profit.df, aes(x=lambda.vec)) +geom_line(aes(y=profitx_h.vec), linetype="dotdash", linewidth=1.2, color="blue") +geom_line(aes(y=profitx_f.vec), linetype="longdash", linewidth=1.2, color="red") +geom_line(aes(y=profitx_h.vec + profitx_f.vec), linetype="solid", linewidth=1.2, color="black") + scale_x_continuous(breaks = seq(0,1,0.1)) + scale_y_continuous(breaks = seq(0,600,50)) +labs(x=TeX("Probability of tariff war: $\\lambda$"), y=TeX("Expected profits of firm  $X$: $E \\pi^X$, $E \\pi_H^X$, $E \\pi_F^X$")) +theme(axis.text.x = element_text(size = 14, color = "black"),  axis.text.y = element_text(size = 16, color = "black"), text = element_text(size = 20)) +geom_vline(xintercept = lambda_hat, color = "black") +annotate("text", x = lambda_hat+0.013, y = 445, label = TeX("$\\hat{\\lambda}$"), size = 8, color="black") +annotate("text", x = 0.3, y = 520, label = TeX("$E \\pi^X=E \\pi^Y$"), size = 8, color="black") +annotate("text", x = 0.3, y = 360, label = TeX("$E \\pi_H^X =E \\pi_F^Y$"), size = 8, color="blue") +annotate("text", x = 0.3, y = 200, label = TeX("$E \\pi_F^X =E \\pi_H^Y$"), size = 8, color="red")
#End Figure 1####
...
#
# Verify Assumption 4 (right term)
...
ggplot(profit_ad.df, aes(x=lambda.vec)) +geom_line(aes(y=profitx_h_ad.vec), linetype="dotdash", linewidth=1.2, color="blue") +geom_line(aes(y=profitx_f_ad.vec), linetype="longdash", linewidth=1.2, color="red") +geom_line(aes(y=profitx_h_ad.vec + profitx_f_ad.vec), linetype="solid", linewidth=1.2, color="black") + scale_x_continuous(breaks = seq(0,1,0.1)) + scale_y_continuous(breaks = seq(0,600,50)) +labs(x=TeX("Probability of ad-valorem tariff war: $\\lambda$"), y=TeX("Expected profits of firm  $X$: $E \\pi^X$, $E \\pi_H^X$, $E \\pi_F^X$")) +theme(axis.text.x = element_text(size = 14, color = "black"),  axis.text.y = element_text(size = 16, color = "black"), text = element_text(size = 20)) +geom_vline(xintercept = lambda_hat, color = "black") +annotate("text", x = lambda_hat_ad+0.017, y = 445, label = TeX("$\\hat{\\lambda}$"), size = 8, color="black") +annotate("text", x = 0.3, y = 525, label = TeX("$E \\pi^X=E \\pi^Y$"), size = 8, color="black") +annotate("text", x = 0.3, y = 355, label = TeX("$E \\pi_H^X =E \\pi_F^Y$"), size = 8, color="blue") +annotate("text", x = 0.3, y = 215, label = TeX("$E \\pi_F^X =E \\pi_H^Y$"), size = 8, color="red")
#End Figure 2####
